...
public class MyPlacesDBAdapter {
    ...
    public MyPlacesDBAdapter(Context context) {
        ...
        this.dbHelper = new MyPlacesDatabaseHelper(context, DATABASE_NAME, null, DATABASE_VERSION);
    }
    ...
    public long insertEntry(MyPlace myPlace) {
        ...
        db.beginTransaction();
        try {
            id = db.insert(DATABASE_NAME, null, contentValues);
    ...
    public boolean removeEntry(long id) {
        ...
        db.beginTransaction();
        try {
            success = db.delete(DATABASE_NAME, PLACE_ID + " =" + id, null) > 0;
            db.setTransactionSuccessful();
        } catch (SQLException e) {
            Log.v("MyPlacesDBAdapter", e.getMessage());
        } finally {
